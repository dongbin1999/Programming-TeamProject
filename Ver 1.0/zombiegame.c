...
#define GunSound ".\\총소리 12.wav"
#define ReloadSound ".\\총장전 7.wav"
...
void sound_gun(void); //총 소리 구현
void sound_reload(void); //장전 소리 구현
...
void sound_gun(void) {
    PlaySound(NULL, 0, 0); //이전 총소리가 있다면 소리 재생을 멈춤
    PlaySound(TEXT(GunSound), 0, SND_ASYNC); //총소리 재생
}

void sound_reload(void) {
    PlaySound(NULL, 0, 0); //이전 장전소리가 있다면 소리 재생을 멈춤
    PlaySound(TEXT(ReloadSound), 0, SND_ASYNC); //장전소리 재생
}